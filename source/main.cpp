...
// TODO: Static Member Variable Initialization should put somewhere less
// conspicuous
SDL_Renderer* MegaBase::renderer = NULL;
Point* MegaBase::offset = NULL;

// TODO: EDGEMAP EDGEMAP EDGEMAP -> MAP EDGES OF EACH SHAPE AND ESTABLISH
// SECTORS FROM THAT
int main(int argc, char* argv[]) {
  LOG("Section: Setup");
  // TODO: Write command line args like in source/idtech1, in addition to
  // command line args such as DRAW_PATHS_ENABLE
  ...
  srand(time(NULL));
  BoundedPoint screenPosition =
      BoundedPoint(Screen::MAX_SCREEN_X_POS, Screen::MAX_SCREEN_Y_POS);
  std::shared_ptr<Dot> dot = std::make_shared<Dot>(Point(190, 150));
  dot->setColorChannels(0xFF);
  Configuration config;
  // TODO: Create a file structure for containing level data so its not
  // hardcoded
  ...
  GAME.createThing<BigWall>(Rect(Line(Point(50, 0), Point(0, 50)),
                                 Line(Point(50, 0), Point(100, 50))));
  ...
  contra.addPlayerKeys(
      popo);  // Maybe allow for multiple bindings of the same command somehow?
              // vectors likely? Also remove this dumb fix
  FpsText fps(
      gFont, Point(100, 10),
      COLORS::RED);  // TODO: Add handler for these things, also have this
                     // singular timer passed to all "groups" for consistency
  handler.addMessage(AlertText("this shouldn't last long", Point(300, 150),
                               COLORS::RED, 20, 2500));
  ...
  std::shared_ptr<SectorPathFollower> foodd =
      GAME.createThing<SectorPathFollower>(
          Rect(GAME.sectors[3]->structure().getCenter(),
               GAME.sectors[3]->structure().getCenter() + Point(10, 10)));
  ...
  while (!contra.quit) {
    ...
    // Testing stuff
    spriteSheetTest.draw("dumb", gRenderer, {200, 200},
                         getDirectionFromAngle(dot->getAngle()));
...
SDL_Window* createWindow() {
  return SDL_CreateWindow(Screen::WINDOW_TITLE.c_str(), Screen::DEFAULT_POS,
                          Screen::DEFAULT_POS, Screen::SCREEN_WIDTH,
                          Screen::SCREEN_HEIGHT,
                          Screen::WINDOW_ARGUMENTS | SDL_RENDERER_PRESENTVSYNC);
}
