...
Configuration::Configuration() {
    ...
    if (this->keyMap["resolution"] != "") {
        ...
        this->configMap["Height"] =
            stoi(this->keyMap["resolution"].substr(fo + 1));
    }
}
...
void Configuration::output(std::ostream& output) {
    for (std::map<std::string, std::string>::iterator iter = keyMap.begin();
         iter != keyMap.end(); iter++) {
        output << iter->first << " = " << iter->second << std::endl;
    }
}

void Configuration::reload() { *this = Configuration(); }
