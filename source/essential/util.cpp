...
void boxRGBA(SDL_Renderer* renderer, Point pointA, Point pointB, uint8_t r,
             uint8_t g, uint8_t b, uint8_t a) {
    boxRGBA(renderer, pointA.x, pointA.y, pointB.x - 1, pointB.y - 1, r, g, b,
            a);
}
...
int keyCodeFromEvent(SDL_Event event) { return event.key.keysym.sym; }
...
int threadTest(void* ptr) {
    ...
    for (int i = 0; i < 10; i++) {
        SDL_Delay(250);
        std::cout << "It's been " << i * 250 << " ms since i started."
                  << std::endl;
    }
