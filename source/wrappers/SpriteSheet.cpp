...
SpriteSheet::SpriteSheet(const std::string& filename, int width, int height,
  ...
  if (this->maxY != 8) {
    LOG("Improperly formatted SpriteSheet! Expected 8 rows, got %i rows.",
        this->maxY);
  }
}
...
bool SpriteSheet::draw(std::string tag, SDL_Renderer* renderer, Point position,
                       EntityDir dir) {
  ...
  if (index >= this->maxX) {
    LOG("ERROR: Sprite Sheet Index out of bounds! %i is greater than the width "
        "of %i!",
        index, this->maxX);
    return false;
  }
...
void SpriteSheet::addAnimation(std::string tag, Uint startingIndex,
                               Uint endingIndex, Uint interval) {
  if (this->animations[tag].isReal()) return;
  if (startingIndex >= this->maxX || endingIndex >= this->maxX) {
    LOG("ERROR: Sprite Sheet Index out of bounds! %i or %i is greater than the "
        "width of %i!",
        startingIndex, endingIndex, this->maxX);
    return;
  }
