...
class Texture {
   ...
   public:
    ...
    void draw(SDL_Renderer* renderer, Point position,
              SDL_COPY_EX_ARGS_DEFAULTS);
    void drawCentered(SDL_Renderer* renderer, Point position,
                      SDL_COPY_EX_ARGS_DEFAULTS);
    bool isLoaded();
    bool notLoaded();
    void createBlank(SDL_Renderer* renderer, int w, int h,
                     Uint32 color = 0x0000000FF);
    SDL_Texture* getRawTexture();

    void loadFromFile(SDL_Renderer* renderer, std::string path, int xSize = 0,
                      int ySize = 0);
    ...
    static Texture scaleTextureBy(SDL_Renderer* renderer, Texture texture,
                                  double xFactor, double yFactor);

    // TODO: Add more Filters
    // Filters
    void bilateralFilter(const float valI, const float valS,
                         const int kernelSize = 5, const int xStart = 2,
                         const int yStart = 2, int width = 0, int height = 0);
    void dither();
    void testFilter();
    void floatyEdges();
};
