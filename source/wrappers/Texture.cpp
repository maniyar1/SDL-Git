...
void Texture::drawCentered(SDL_Renderer* renderer, Point position,
  ...
  if (!this->width || !this->height)
    SDL_QueryTexture(this->texture, NULL, NULL, &this->width, &this->height);
  this->draw(renderer, position - Point(this->width, this->height) / 2.0, clip,
             angle, center, flip);
}
        ...
        for (int i = 0; i < kernelSize; i++) {
          for (int j = 0; j < kernelSize; j++) {
            ...
            float gaussIR = gaussian(
                pixels[otherX][otherY].red() - pixels[x][y].red(), valI);
            float gaussS = gaussian(
                Point(x, y).distanceToPoint(Point(otherX, otherY)), valS);
            double deltaR = gaussIR * gaussS;
            totalR += pixels[otherX][otherY].red() * deltaR;
            weightR += deltaR;

            float gaussIG = gaussian(
                pixels[otherX][otherY].green() - pixels[x][y].green(), valI);
            double deltaG = gaussIG * gaussS;
            totalG += pixels[otherX][otherY].green() * deltaG;
            weightG += deltaG;

            float gaussIB = gaussian(
                pixels[otherX][otherY].blue() - pixels[x][y].blue(), valI);
