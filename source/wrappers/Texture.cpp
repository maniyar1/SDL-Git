...
SDL_Texture* Texture::getBlankRenderTarget(SDL_Renderer* renderer) {
    ...
    return SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA32,
                             SDL_TEXTUREACCESS_TARGET, this->width,
                             this->height);
}
...
void Texture::setColorKey(Uint8 red, Uint8 green,
                          Uint8 blue) {  // Modified from lazyfoo.net
    if (this->texture == NULL) return;
...
void Texture::draw(SDL_Renderer* renderer, Point position, SDL_COPY_EX_ARGS) {
    ...
    if (!this->width || !this->height)
        SDL_QueryTexture(this->texture, NULL, NULL, &this->width,
                         &this->height);
    SDL_Rect renderQuad = {(int)position.x, (int)position.y, this->width,
                           this->height};
    if (clip != NULL) {
        renderQuad.w = clip->w;
        renderQuad.h = clip->h;
...
void Texture::drawCentered(SDL_Renderer* renderer, Point position,
                           SDL_COPY_EX_ARGS) {
    if (!this->width || !this->height)
        SDL_QueryTexture(this->texture, NULL, NULL, &this->width,
                         &this->height);
    this->draw(renderer, position - Point(this->width, this->height) / 2.0,
               clip, angle, center, flip);
}

bool Texture::isLoaded() { return this->texture != NULL; }

bool Texture::notLoaded() { return this->texture == NULL; }
...
SDL_Texture* Texture::getRawTexture() { return this->texture; }

void Texture::loadFromFile(SDL_Renderer* renderer, std::string path, int xSize,
                           int ySize) {
    if (this->renderer == NULL) this->renderer = renderer;
    this->free();
...
void Texture::normalizeTexture(SDL_Renderer* renderer) {
    ...
    // Method to take a SDL_TEXTUREACCESS_TARGET and make it a
    // SDL_TEXTUREACCESS_STREAMING for pixel modification
    int access, width, height;
    SDL_QueryTexture(this->texture, NULL, &access, &width, &height);
    if (access == SDL_TEXTUREACCESS_STREAMING) {
        return;
    } else if (access ==
               SDL_TEXTUREACCESS_STATIC) {  // Don't know what to do with it :(
        // LOG("Texture was created with the Texture Access of Static -> Fix
        // This");
        ...
        int streamingPitch;
        SDL_LockTexture(streamingTexture, NULL, &streamingPixels,
                        &streamingPitch);
...
Texture Texture::scaleTextureBy(SDL_Renderer* renderer, Texture texture,
...
void Texture::bilateralFilter(float valI, float valS, const int kernelSize,
                              const int xStart, const int yStart, int width,
                              int height) {
    if (this->texture == NULL) return;
    PixelMod mod(this->texture, true);
    if (mod.notLocked()) return;
    {
        Pixel pixels[mod.width()]
                    [mod.height()];  // Using a static array is not only more
                                     // elegant, it's also more performant
            ...
            for (int y = yStart; y - yStart < height; y++) {
                ...
                for (int i = 0; i < kernelSize; i++) {
                    for (int j = 0; j < kernelSize; j++) {
                        int otherX = x - (half - i);
                        int otherY = y - (half - j);
                        float gaussIR = gaussian(
                            pixels[otherX][otherY].red() - pixels[x][y].red(),
                            valI);
                        float gaussS = gaussian(
                            Point(x, y).distanceToPoint(Point(otherX, otherY)),
                            valS);
                        ...
                        float gaussIG =
                            gaussian(pixels[otherX][otherY].green() -
                                         pixels[x][y].green(),
                                     valI);
                        ...
                        float gaussIB = gaussian(
                            pixels[otherX][otherY].blue() - pixels[x][y].blue(),
                            valI);
