...
PixelMod::PixelMod(SDL_Texture* texture, bool wrapEdges) : texture(texture) {
    ...
    SDL_QueryTexture(this->texture, &format, NULL, &this->textureWidth,
                     &this->textureHeight);
...
bool PixelMod::notLocked() { return this->unlocked; }

uint32& PixelMod::at(int x, int y) {
    if (x < 0 || x > (this->width() - 1) || y < 0 || y > (this->height() - 1)) {
        // If the requested position is outside of the array return a blank
        // pixel with no data in it
        if (this->edges) {
            if (x < 0) {
                x += this->width();
            }
...
int PixelMod::count() const { return this->pixelCount; }

int PixelMod::height() const { return this->textureHeight; }

int PixelMod::pitch() const { return this->texturePitch; }

int PixelMod::width() const { return this->textureWidth; }

uint32 PixelMod::mapRGBA(const uint8 r, const uint8 g, const uint8 b,
                         const uint8 a) const {
...
uint32& PixelMod::operator[](const int index) { return this->pixels[index]; }
