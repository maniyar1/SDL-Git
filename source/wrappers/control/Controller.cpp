...
int scanCodeFromEvent(SDL_Event event) { return event.key.keysym.scancode; }
...
void Controller::handleEvents() {
        ...
        switch (e.type) {
            ...
            case SDL_KEYDOWN:
                if (e.key.repeat == 0) {
                    ...
                    if (scanCodeFromEvent(e) ==
                        this->config["Exit"]) {  // Band-aid fix
                        this->quit = true;
                    }
                }
                break;
    ...
    std::map<int, std::shared_ptr<ButtonCommand>>::iterator iterator =
        buttons.begin();
...
void Controller::addButton(std::string str,
                           std::shared_ptr<ButtonCommand> button) {
...
void Controller::updateListeners() {
    for (std::map<int, HeldKey>::iterator iterator = this->listeners.begin();
         iterator != listeners.end(); iterator++) {
        iterator->second.set(this->stuff[iterator->first]);
    }
}

HeldKey& Controller::checkListener(int key) { return this->listeners[key]; }

void Controller::addPlayerKeys(Point& target) {
    this->addButton(config["Right"], std::make_shared<PlayerMoveCommand>(
                                         BASIC::PLAYER_RIGHT_KEYDOWN, &target));
    this->addButton(config["Left"], std::make_shared<PlayerMoveCommand>(
                                        BASIC::PLAYER_LEFT_KEYDOWN, &target));
    this->addButton(config["Up"], std::make_shared<PlayerMoveCommand>(
                                      BASIC::PLAYER_UP_KEYDOWN, &target));
    this->addButton(config["Down"], std::make_shared<PlayerMoveCommand>(
                                        BASIC::PLAYER_DOWN_KEYDOWN, &target));
}
