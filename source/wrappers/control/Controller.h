...
/* Class to handle event handling(haha) less messily and with easier
 * functionality to implement new events */
class Controller {
    /* There are three types of events the Controller will handle; Keys,
     * Buttons, and Listeners Keys are for commands that have distinct states of
     * down and up Buttons are for actions that only activate when they key is
     * held down, but don't care when released Listeners are an extention of
     * HeldKey for more complicated external usage
     */
   private:
   ...
   public:
    ...
    void addCheat(
        std::string key,
        void (*func)());  // TODO: Make this not dependent on function pointers
};
#endif
