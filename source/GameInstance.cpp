...
GameInstance::GameInstance(SDL_Renderer* renderer, BoundedPoint offset)
    : offset(offset),
      playableArea(0, 0, Screen::MAX_WIDTH, Screen::MAX_HEIGHT),
      renderer(renderer),
      collision(this),
      sectors(this) {}
...
void GameInstance::update() {
    // TODO: Collision by objects in sectors, mid tier priority due to requiring
    // sizeable reworking
    for (ThingPtr& thing : this->movingThings) {
        Point position = thing->getPosition();
        thing->update();
        // If the object has moved, its relative draw order might need to be
        // adjusted
...
Rect GameInstance::getPlayableArea() const { return this->playableArea; }

SDL_Renderer* GameInstance::getRenderer() { return this->renderer; }

Point& GameInstance::getOffset() { return this->offset; }
...
ThingPtr GameInstance::getPlayer() { return this->PLAYER; }
