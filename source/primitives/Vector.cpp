...
Vector Vector::operator/(const double& num) { return (*this) * (1.0 / num); }

double Vector::dot(const Vector other) {
  return (this->x * other.x) + (this->y * other.y);
}

double Vector::operator*(const Vector& other) { return this->dot(other); }
...
Vector operator*(const double& num, Vector& other) { return other * num; }
