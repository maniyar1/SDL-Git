...
Line::Line(Point pointA, Point pointB, uint8_t r, uint8_t g, uint8_t b,
           uint8_t a)
    : originPoint(pointA), endingPoint(pointB) {
    if (pointA.isNull()) {
...
Line::Line(const Line& line)
    : originPoint(line.originPoint), endingPoint(line.endingPoint) {
...
bool Line::isCollinear(const Line& other) const {
    ...
    if (this->isParallel(other)) {
        return std::abs(other.getC() * this->getBy() -
                        this->getC() * other.getBy()) < 0.00001;
    }
    return false;
}
...
bool Line::isParallel(const Line& other) const {
    ...
    return std::abs((other.getAx() * this->getBy()) -
                    (this->getAx() * other.getBy())) < 0.0001;
}
...
bool Line::isPointOnThisLine(const Point& point) const {
    ...
    return valueInRange(point.x, minX, maxX) &&
           valueInRange(point.y, minY, maxY);
}
...
void Line::operator-=(const Point& b) { *this += b.negate(); }
...
double Line::getC() const {
    return double((this->getAx() * this->originPoint.x +
                   (this->getBy() * this->originPoint.y)));
}

double Line::getMagnitude() const { return sqrt(this->getFastMagnitude()); }

double Line::getFastMagnitude() const {
    return (this->endingPoint - this->originPoint).getFastMagnitude();
}

Point Line::getEnd() const { return this->endingPoint; }

Point Line::getOrigin() const { return this->originPoint; }

Point Line::getUnitVector() const { return this->getVector().getUnitVector(); }

Point Line::getVector() const { return this->endingPoint - this->originPoint; }
...
void Line::drawLine(SDL_Renderer* renderer, Point offset) {
    aalineRGBA(renderer, this->originPoint - offset, this->endingPoint - offset,
...
Point Line::intersectionPoint(const Line& other) const {
    double delta =
        (this->getAx() * other.getBy()) - (this->getBy() * other.getAx());
    if (delta == 0) {
        return Point();
    }
    double x =
        ((this->getC() * other.getBy()) - (this->getBy() * other.getC())) /
        delta;
    double y =
        ((this->getAx() * other.getC()) - (this->getC() * other.getAx())) /
        delta;
...
Line Line::operator*(const double& delta) const {
    return Line(this->getOrigin(),
                this->getOrigin() + this->getVector() * delta);
}
...
Line operator*(const double& delta, const Line& line) { return line * delta; }
...
bool Line::shareNoPoints(const Line& other) const {
    return this->originPoint != other.originPoint &&
           this->originPoint != other.endingPoint &&
           this->endingPoint != other.originPoint &&
