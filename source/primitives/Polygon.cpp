...
bool Polygon::overlap(const Polygon& that) const {
  ...
  // It will be more efficient to check fewer points in the later stages of the
  // algorithm
  ...
  for (Point point : that.getPoints()) {
    // Even-odd rule, if the number of collisions with a polygon of a line is
    // odd, then the starting point IS inside the polygon
    if (this->numberOfCollisions(Line(point, OUT_OF_BOUNDS)) & 1) return true;
  }
  // Default case where the bounding rectangles overlap but no points are inside
  // the other polygon
  return false;
}
