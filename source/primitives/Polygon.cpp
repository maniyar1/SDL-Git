...
bool Polygon::isAxisAligned() const { return false; }
...
bool Polygon::overlap(const Polygon& that) const {
    ...
    // It will be more efficient to check fewer points in the later stages of
    // the algorithm
    ...
    // Shorthand variables to store topleft coordinates of both bounding
    // rectangles
    ...
    bool xOver = valueInRange(myXPos, otXPos, otXPos + other.getWidth()) ||
                 valueInRange(otXPos, myXPos, myXPos + me.getWidth());
    bool yOver = valueInRange(myYPos, otYPos, otYPos + other.getHeight()) ||
                 valueInRange(otYPos, myYPos, myYPos + me.getHeight());
    // If the bounding rectangles don't overlap, then the polygons can't overlap
    if (!(xOver && yOver)) return false;
    // If the bounding rectangles DO overlap, and both polygons are THEIR OWN
    // bounding rectangles, they must overlap
    if (this->isAxisAligned() && that.isAxisAligned()) return true;
    for (Line line : this->getLines()) {
        if (that.doesLineCollide(line)) return true;
    }
    // If the bounding rectanlges DO overlap, AND none of the lines overlap AND
    // the polygons DO overlap, then at LEAST one point is inside of the other
    for (Point point : that.getPoints()) {
        // Even-odd rule, if the number of collisions with a polygon of a line
        // is odd, then the starting point IS inside the polygon
        if (this->numberOfCollisions(Line(point, OUT_OF_BOUNDS)) & 1)
            return true;
    }
    // Default case where the bounding rectangles overlap but no points are
