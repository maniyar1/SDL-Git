...
double Point::originDistance() const { return this->distanceToPoint(0, 0); }
...
void Point::operator-=(const Point& delta) { *this += delta.negate(); }
...
Point Point::rotate90() const { return Point(this->y, -this->x); }

Point Point::toInt() const { return Point(int(this->x), int(this->y)); }
...
double Point::getAngle() const { return atan2(this->y, this->x); }

bool Point::isNull() const { return this->x != this->x && this->y != this->y; }

bool Point::isZero() const { return !this->getNonZero(); }

bool Point::isReal() const { return !isNull(); }

Point Point::copy() const { return Point(this->x, this->y); }

void Point::xZero() { this->x = 0; }

void Point::yZero() { this->y = 0; }
...
bool Point::getNonZero() const {
    return ((std::abs(this->x) > ROUNDING || std::abs(this->y) > ROUNDING) &&
            (this->x == this->x ||
             this->y == this->y));  // TODO: Define accuarcy constant
}

bool Point::operator==(const Point& point) const {
    return std::abs(point.x - this->x) < ROUNDING &&
           std::abs(point.y - this->y) < ROUNDING;
}

bool Point::operator!=(const Point& point) const { return !(*this == point); }

Point Point::negate() const { return Point(-this->x, -this->y); }

Point Point::onlyX() const { return Point(this->x, 0); }

Point Point::onlyY() const { return Point(0, this->y); }
