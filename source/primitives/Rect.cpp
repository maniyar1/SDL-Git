...
Rect::Rect(Point topLeft, Line widthVector, Line heightVector)
    : topLeft(topLeft),
      heightVector(heightVector.getVector()),
      widthVector(widthVector.getVector()) {
    // Point and two magnitude vectors
}

Rect::Rect(Point topLeft, Point widthVector, Point heightVector)
    : topLeft(topLeft), heightVector(heightVector), widthVector(widthVector) {}
...
Rect::Rect(Line side1, Line side2) {
    ...
    if (side1.isOrthogonal(side2)) {  // Ideal
        ...
        if (distance1 > distance2) {
            this->widthVector =
                Line(side1.getOrigin(), side2.getEnd()).getVector();
        } else {
            this->widthVector =
                Line(side1.getOrigin(), side2.getOrigin()).getVector();
        }
    } else {  // shit
    ...
    Point check[] = {side1.getOrigin(), side1.getEnd(), side2.getOrigin(),
                     side2.getEnd()};
    while (true) {
        Point ar[] = {this->getTopLeft(), this->getTopRight(),
                      this->getBottomRight(), this->getBottomLeft()};
        int ctr = 0;
        for (Point g : check) {
            bool f = false;
            for (Point p : ar) {
...
bool Rect::isAxisAligned() const {  // Fuck off axis powers
    return (this->heightVector.x == 0 && this->widthVector.y == 0) ||
           (this->heightVector.y == 0 && this->widthVector.x == 0);
}
...
int Rect::numLines() const { return 4; }

int Rect::numPoints() const { return 4; }

Point Rect::collideLine(const Line& ray) const {
    /* Returns the point where the line intersects the rect, if it doesn't
     * intersect it returns a null point */
    Point intersection, tempPoint;
    for (Line line : this->getLines()) {
        tempPoint = line.intersectionPoint(ray);
        if (tempPoint.isReal()) {
            if (intersection.isNull() ||
                tempPoint.distanceToPoint(ray.getOrigin()) <
                    intersection.distanceToPoint(ray.getOrigin())) {
...
std::vector<Point> Rect::getPoints() const {
    return {this->getTopLeft(), this->getTopRight(), this->getBottomRight(),
            this->getBottomLeft()};
}
...
void Rect::draw(SDL_Renderer* renderer, Point offset) {
    ...
    polygonRGBA(renderer, x, y, temp.size(), this->rChannel, this->bChannel,
                this->gChannel, this->aChannel);
    delete[] x;
    delete[] y;
}
...
Point Rect::getTopLeft() const { return this->topLeft; }

Point Rect::getTopRight() const { return this->topLeft + this->widthVector; }

Point Rect::getBottomLeft() const { return this->topLeft + this->heightVector; }

Point Rect::getBottomRight() const {
    return this->getBottomLeft() + this->widthVector;
}

double Rect::getWidth() const { return this->widthVector.getMagnitude(); }
double Rect::getHeight() const { return this->heightVector.getMagnitude(); }

SDL_Rect Rect::getSDLRect() const {
    SDL_Rect tempRect = {int(this->topLeft.x), int(this->topLeft.y),
                         int(this->widthVector.getMagnitude()),
                         int(this->heightVector.getMagnitude())};
    return tempRect;
}
...
Rect Rect::operator*(const double& value) const {
    Rect temp(this->topLeft, this->widthVector * value,
              this->heightVector * value);
    temp.setCenter(this->getCenter());
    return temp;
}

void Rect::operator+=(const Point& point) { this->topLeft += point; }

void Rect::operator-=(const Point& point) { *this += point.negate(); }

void Rect::operator*=(const double& value) { *this = *this * value; }

Point Rect::getCenter() const {
    return this->topLeft + (this->widthVector + this->heightVector) / 2;
}

Rect operator+(const Point& point, const Rect& rect) { return rect + point; }

Rect operator-(const Point& point, const Rect& rect) { return rect - point; }

Rect operator*(const double& value, const Rect& rect) { return rect * value; }
