...
Dot::Dot(Point startingCoordinate) : EntityBase(DRAW | MOVEABLE) {
  ...
  this->setFriction(10);
  this->position = BoundedPoint(
      startingCoordinate, 0, 0, Screen::MAX_WIDTH - Player::PLAYER_X_DIMENSION,
      Screen::MAX_HEIGHT - Player::PLAYER_Y_DIMENSION);
}
...
Point Dot::getCenter() {
  return this->position +
         (Point(Player::PLAYER_X_DIMENSION, Player::PLAYER_Y_DIMENSION) / 2);
}
...
Point Dot::getPosition() const { return this->position; }

Rect Dot::getBoundingRect() const {
  return Rect(this->position, Player::PLAYER_X_DIMENSION,
              Player::PLAYER_Y_DIMENSION);
}

Line Dot::getRay() {
  Point temp = Point(this->getCenter());
  temp += Point(Player::PLAYER_RAY_CAST_LENGTH * cos(this->angle),
                Player::PLAYER_RAY_CAST_LENGTH * sin(this->angle));
  return Line(this->getCenter(), temp);
}

float Dot::getAngle() { return this->angle; }
...
void Dot::draw(SDL_Renderer* renderer, Point offset) {
  ...
  Rect p(temp.x, temp.y, Player::PLAYER_X_DIMENSION,
         Player::PLAYER_Y_DIMENSION);
...
void Dot::update() { this->collideTest(); }
...
void Dot::collideTest() {
  ...
  for (int i = 0; i < CHECKS; i++) {
    ...
    if (this->parent->collision.doesNotCollideWith(this->getBoundingRect() +
                                                   temp)) {
      this->move(temp);
      xDelta += temp.x;
      yDelta += temp.y;
    ...
    if (not xDelta) {
            if
    (this->parent->collision.doesNotCollideWith(this->getBoundingRect() +
    temp.onlyX())) { xDelta = temp.x(); this->parent->getOffset() +=
    temp.onlyX();
            }
    }
    if (not yDelta) {
            if
    (this->parent->collision.doesNotCollideWith(this->getBoundingRect() +
    temp.onlyY())) { yDelta = temp.y(); this->parent->getOffset() +=
    temp.onlyY();
            }
    }
    if (xDelta && yDelta) {
  ...
  // PUT THIS ELSEWHERE <- Will be handled when implementation is changed to
  // being based around the Screen Class
  ...
  if (this->getPosition().x > Screen::MAX_X_SCROLL_DISTANCE) {
    this->parent->getOffset() =
        Point(Screen::MAX_SCREEN_X_POS, this->parent->getOffset().y);
  }
  if (this->getPosition().y > Screen::MAX_Y_SCROLL_DISTANCE) {
    this->parent->getOffset() =
        Point(this->parent->getOffset().x, Screen::MAX_SCREEN_Y_POS);
  }
}

void Dot::rayCast() {
  Point newPoint =
      this->parent->collision.closestPointThatCollidesWith(this->getRay());
  if (newPoint.isReal()) {
...
int tempF(double val) { return ((val - int(val) > (1 - .05)) ? 1 : 0); }

Point tempF(Point point) { return Point(tempF(point.x), tempF(point.y)); }
