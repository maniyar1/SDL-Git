...
SectorPathFollower::SectorPathFollower(Rect rect)
    : ThingBase(DRAW | MOVEABLE), box(rect), mine(this) {}
...
bool SectorPathFollower::overlap(
    const std::shared_ptr<ThingBase>& other) const {
...
Point SectorPathFollower::getPosition() const { return this->box.getCenter(); }

Rect SectorPathFollower::getBoundingRect() const { return this->box; }

void SectorPathFollower::draw(SDL_Renderer* renderer, Point offset) {
  if (!this->texture.isLoaded())
    this->texture.loadFromFile(renderer, "resources/temp.png", 10, 10);
  this->mine.draw();
