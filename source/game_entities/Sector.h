...
class SectorBase {
   public:
    ...
    virtual void connectToOthers(
        std::vector<std::shared_ptr<SectorBase>>& others) = 0;
    virtual void clean(std::vector<std::shared_ptr<SectorBase>>& others) = 0;
    virtual void draw() = 0;
};

template <class T>
class Sector : public SectorBase {
    static_assert(std::is_base_of<Polygon, T>::value,
                  "Sector must be templated with a parameter of type polygon.");
   ...
   public:
    ...
    void connectToOthers(
        std::vector<std::shared_ptr<SectorBase>>& others) override;
    void clean(std::vector<std::shared_ptr<SectorBase>>& others) override;
    void draw() override;
};

template <class T>
Sector<T>::Sector(T structure, std::string data)
    : representation(structure), data(data) {}

template <class T>
Sector<T>::~Sector() {}

template <class T>
bool Sector<T>::contains(SectorBase* pointer) const {
...
template <class T>
Polygon& Sector<T>::structure() {
    return this->representation;
}

template <class T>
std::map<SectorBase*, Point>& Sector<T>::pointsOfContact() {
    return this->contact;
}

template <class T>
std::string Sector<T>::getData() const {
    return this->data;
}

template <class T>
std::vector<std::weak_ptr<SectorBase>>& Sector<T>::attached() {
    return this->connected;
}

template <class T>
std::vector<std::weak_ptr<ThingBase>>& Sector<T>::getContained() {
    return this->containedThings;
}

template <class T>
void Sector<T>::connectToOthers(
    std::vector<std::shared_ptr<SectorBase>>& others) {
    std::vector<Line> lines;
    {
        ...
        for (Line line : this->representation.getLines()) {
            ...
            lines.push_back(
                (positive.getFastMagnitude() > negative.getFastMagnitude())
                    ? positive
                    : negative);
        }
    }
    for (std::shared_ptr<SectorBase>& sector : others) {
        if (sector.get() == this || this->contains(sector.get()))
            continue;  // Shouldn't happen but edge cases :D
        for (Line line : lines) {
...
template <class T>
void Sector<T>::clean(std::vector<std::shared_ptr<SectorBase>>& others) {
    for (std::shared_ptr<SectorBase>& sector : others) {
        if (sector->contains(this) && !this->contains(sector.get())) {
            this->connected.push_back(sector);
        }
        if (this->contains(sector.get()) &&
            this->contact[sector.get()].isNull()) {
