...
BadTest::BadTest(Point position) : EnemyBase(position, DRAW | MOVEABLE) {
    ...
    int dist = 200;
    this->c.addPath(
        std::make_shared<LinePath>(Point(50, 0), dist, Path::SINGLE_LOOP));
...
bool BadTest::isLocationInvalid() const {
    /* True  -> Invalid location, collision or some other predefined metric
     * doesn't satisfy False -> Valid location */
    return this->parent->collision.doesCollideWith(
...
void BadTest::draw(SDL_Renderer* renderer, Point offset) {
    ...
    for (int i = -20; i <= 20; i++) {
            ...
            pTemp += Point(300 * cos(this->angle + radians(i)), 300 *
    sin(this->angle + radians(i))); Line temp = Line(this->getCenter(), pTemp);

            Point newTemp =
    this->parent->collision.closestPointThatCollidesWith(temp); if
    (newTemp.isReal()) { temp = Line(this->getCenter(), newTemp);
            }
            temp.setColorChannels(COLORS::BLACK);
            temp.drawLine(this->parent->getRenderer(),
...
void BadTest::update() {
    ...
    if (this->currentState == State::PATROL || this->currentState ==
    State::GOTO) { for (int i = -20; i <= 20; i++) { Point pTemp =
    this->getCenter(); pTemp += Point(cos(this->angle + radians(i)),
    sin(this->angle + radians(i))) * 300; Line temp = Line(this->getCenter(),
    pTemp);
    ...
    (newTemp.isReal()) { temp = Line(this->getCenter(), newTemp);
                    ...
                    if
    (this->parent->getPlayer()->getRect().doesLineCollide(temp)) {
                            this->targetPoint =
    this->parent->getPlayer()->getPosition(); if (this->currentState ==
    State::PATROL) {
                                    //std::cout << "NOW GOTO: " <<
    this->position
    << std::endl; this->lastPatrolledPoint = this->getCenter(); this->c.pause();
                            }
...
Point BadTest::getPosition() const { return this->position; }
