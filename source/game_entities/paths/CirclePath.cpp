...
CirclePath::CirclePath(int radius, float periodModify, int maxTicks,
                       bool clockwise, int startingTicks)
    : Path() {
...
void CirclePath::modify(float time) {
    float tempdx = (this->outsideMult) *
                   cos(this->ticksDone * M_PI / 180.f * this->periodModify);
    float tempdy = (this->plot * this->outsideMult) *
                   sin(this->ticksDone * M_PI / 180.f * this->periodModify);
