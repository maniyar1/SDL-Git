...
ZeroRadiusTurnPath::ZeroRadiusTurnPath(float targetAngle, float time,
                                       bool clockwise)
    : Path(), targetAngle(targetAngle), clockwise(clockwise) {
...
void ZeroRadiusTurnPath::modify(float delta) {
    ...
    float deltaAngle =
        (this->targetAngle - this->startingAngle) / (this->maxTicks * 10);
    if (this - clockwise) {
