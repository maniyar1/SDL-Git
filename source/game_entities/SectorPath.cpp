...
double getValue(SectorPtr sector, SectorPtr target) {
    return sector->structure().getCenter().fastDistanceToPoint(
               target->structure().getCenter()) *
           ((sector->contains(target.get()) ? .25 : 1));
}

double edgeFunction(SectorPtr sector, SectorPtr target) {
    return sector->pointsOfContact()[target.get()].fastDistanceToPoint(
        target->structure().getCenter());
}

SectorPath::SectorPath(ThingBase* target) : target(target) {}

SectorPath::~SectorPath() { this->clear(); }
...
bool SectorPath::isFinished() const { return this->stored.size() <= 1; }

int SectorPath::size() { return this->stored.size(); }

Point SectorPath::currentTarget(Point currentPosition) {
    Point centerDelta =
        (this->stored[0]->structure().getCenter() - currentPosition);
    switch (this->stored.size()) {
        ...
        default:
            Point edgePoint =
                this->stored[0]->pointsOfContact()[this->stored[1].get()];
...
void SectorPath::clear() { this->stored.clear(); }
...
void SectorPath::createPath(SectorPtr startingSector, SectorPtr target) {
    ...
    while (unused.size() > 0) {
        ...
        if (current == target) {
            ...
            this->stored.push_back(
                startingSector);  // This *shouldn't* be necessary but it
                                  // appears to be for the time being
        ...
        for (std::weak_ptr<SectorBase> weak : current->attached()) {
            ...
            if (general < cost[node].value) {
                ...
                currentCost[node].value =
                    cost[node].value + getValue(node, target);
...
void SectorPath::draw() {
    for (uint i = 0; i + 1 < this->stored.size(); i++) {
        Line p(this->stored[i]->structure().getCenter(),
               this->stored[i]->pointsOfContact()[this->stored[i + 1].get()]);
        p.setColor(0xFF, 0x00, 0xFF, 0xFF);
        p.drawLine(MegaBase::renderer, *MegaBase::offset);
        if (i != 0) {
            Line g(
                this->stored[i]->structure().getCenter(),
                this->stored[i - 1]->pointsOfContact()[this->stored[i].get()]);
