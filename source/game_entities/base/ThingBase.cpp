...
void ThingBase::setParent(GameInstance* parent) { this->parent = parent; }

int ThingBase::getAbsoluteFlags() const { return this->flags; }

int ThingBase::getFlags() const { return this->flags; }
...
ENTITY_DIRECTION getDirectionFromAngle(double angle) {
    ...
    if (angle > M_PI_8 && angle < M_PI_4 + M_PI_8) {
        ...
        return UP_LEFT;
    } else if (angle > M_PI + M_PI_2 - M_PI_8 &&
               angle < M_PI + M_PI_2 + M_PI_8) {
        return UP;
