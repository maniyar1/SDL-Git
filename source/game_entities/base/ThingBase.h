...
class ThingBase {
 protected:
  const int absoluteFlags;  // List of attributes this object CAN have, but
                            // might not necessarily have at any given moment
 ...
 public:
   ...
   * overlap(shared_ptr<Thing>) -> Does this object collide with this object(ie
   * call the objects overlap with each hitbox in this */
  virtual bool overlap(const Polygon& other) const = 0;
  virtual bool overlap(const std::shared_ptr<ThingBase>& other) const = 0;
  virtual bool wideOverlap(const Polygon& other)
      const = 0;  // TODO: get rid of this fucking bullshit
