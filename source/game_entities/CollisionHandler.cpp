...
bool CollisionHandler::doesCollideWith(const Polygon& rect,
                                       void* pointer) const {
...
bool CollisionHandler::doesNotCollideWith(const Polygon& rect,
                                          void* pointer) const {
...
bool CollisionHandler::doesNotCollideWith(const Line& line,
                                          void* pointer) const {
...
bool CollisionHandler::doesCollideWith(
    const std::shared_ptr<ThingBase>& thing) const {
    /* True - the rect DOES collide with this collide group
     * False - the rect DOESN'T collide with this collide group */
...
bool CollisionHandler::doesNotCollideWith(
    const std::shared_ptr<ThingBase>& thing) const {
    return !this->doesCollideWith(thing);
}

Point CollisionHandler::closestPointThatCollidesWith(const Line& ray,
                                                     void* pointer) const {
    Point stored, temp;
    for (const ThingPtr& thing : this->parent->collisionThings) {
        ...
        if (stored.isNull() || temp.distanceToPoint(ray.getOrigin()) <
                                   stored.distanceToPoint(ray.getOrigin())) {
            stored = temp;
        }
    }
    return stored;
